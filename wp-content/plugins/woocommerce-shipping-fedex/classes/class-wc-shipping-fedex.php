...
class WC_Shipping_Fedex extends WC_Shipping_Method {
    ...
    public function calculate_shipping( $package ) {
		...
		// HACK
		$only_overnight = false;

		foreach($woocommerce->cart->cart_contents AS $product) {
			$curr_product = get_product($product['product_id']);
			$shipping_class = $curr_product->get_shipping_class();
//			var_dump($curr_product);
//			var_dump($shipping_class);
			if ($shipping_class == 'frozen') { $only_overnight = true; }
		}
		...
		if ( $this->found_rates ) {
			...
			if ( $this->offer_rates == 'all' ) {
				...
				foreach ( $this->found_rates as $key => $rate ) {
					// HACK
					if ($only_overnight) {
						$valid_methods = array('fedex:STANDARD_OVERNIGHT', 'fedex:PRIORITY_OVERNIGHT');
						if (in_array($key, $valid_methods)) {
							$this->add_rate($rate);
						}
					} else {
						$this->add_rate( $rate );
    ...
    private function get_freight_requests( $package_requests, $package ) {
    	...
    	foreach ( $package_requests as $freight_request ) {
    		...
    		$freight_request['RequestedShipment']['Shipper'] = array(
			    'Address'               => array(
			    	'StreetLines'         => strtoupper( $freight_shipper_street ),
	    	...
	    	$freight_request['RequestedShipment']['FreightShipmentDetail'] = array(
				...
				'FedExFreightBillingContactAndAddress' => array(
					'Address'                             => array(
						'StreetLines'                        => strtoupper( $freight_billing_street ),
